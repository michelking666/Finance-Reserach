...
def list_cards(only_saved: bool = False, _: dict = Depends(get_current_user)) -> list[TopicCard]:
    return store.list_cards(only_saved=only_saved)


@router.get("/{card_id}", response_model=TopicCard)
def get_card(card_id: str, _: dict = Depends(get_current_user)) -> TopicCard:
...
def create_card(req: CardCreateRequest, _: dict = Depends(get_current_user)) -> TopicCard:
    card = TopicCard(**req.model_dump(), saved=True)
    return store.upsert_card(card)


@router.patch("/{card_id}/save", response_model=TopicCard)
def save_card(card_id: str, saved: bool = True, _: dict = Depends(get_current_user)) -> TopicCard:
...
def delete_card(card_id: str, _: dict = Depends(get_current_user)) -> dict[str, bool]:
